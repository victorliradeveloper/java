...
- **Backoff exponencial com jitter**: falhas no `publishOne` agendam `next_attempt_at` via politica externa `BackoffPolicy` (default `ExponentialJitterBackoffPolicy`). A entidade nao calcula nada — `markFailed` recebe `IntFunction<LocalDateTime>` resolver e aplica o resultado, mantendo SRP. Sequencia tipica 2s, 4s, 8s, 16s, 32s, 60s (cap) com +/-25% de jitter. `claimNext` filtra docs com `next_attempt_at > now`. Indice composto `(published_at, next_attempt_at, created_at)` na `V005_OutboxNextAttemptIndex`. Config tipada via `OutboxProperties` (`outbox.backoff.initial-ms` / `outbox.backoff.max-ms`).
