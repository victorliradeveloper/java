 ...
 * <ol>
 *   <li>Tenta gravar um claim atomico na tabela <code>idempotency_keys</code>
 *       (INSERT puro via {@link IdempotencyClaimWriter}, nao upsert).</li>
 *   <li>Se inserir (claim novo): executa a operacao, grava o response na linha, retorna.</li>
 ...
 * O claim eh atomico via PK unica em <code>key</code> + {@code persist + flush}
 * em transacao REQUIRES_NEW. Duas chamadas simultaneas com a mesma key: uma
 * vence o insert, a outra cai em {@link DataIntegrityViolationException}
 * (traduzida do SQLSTATE 23505 pelo Spring) e e' tratada no caminho de
 * "ja existe".
 *
 ...
 *
 * <h3>Limpeza</h3>
 * Postgres nao tem TTL nativo como Mongo. Um job @Scheduled
 * ({@link IdempotencyKeyCleanupJob}) deleta linhas com {@code expires_at < now()}
 * periodicamente.
 */
@Slf4j
@Service
public class IdempotencyService {
    ...
    private <T> T runIdempotent(String idempotencyKey,
        ...
        try {
            claimWriter.insert(claim);
        } catch (DataIntegrityViolationException e) {
            // Hibernate nao gera DuplicateKeyException especifica como o Mongo;
            // a unica constraint nessa tabela e' a PK em `key`, entao qualquer
            // violacao de integridade aqui significa "key ja existe".
    ...
    private <T> T replayExisting(String idempotencyKey,
        ...
        IdempotencyKey existing = repository.findById(idempotencyKey)
                .orElseThrow(() -> new IllegalStateException(
                        "Idempotency-Key '" + idempotencyKey + "' sumiu apos DuplicateKeyException — possivel cleanup race"));
    ...
    private void releaseClaimOnFailure(String idempotencyKey, RuntimeException ex) {
        try {
            ...
            log.error("[IDEMPOTENCY] falha ao liberar claim key={} — sera limpo pelo cleanup job",
