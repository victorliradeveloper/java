    ...
    public boolean insertIfAbsent(TodoAuditLog log) {
        Query query = entityManager.createNativeQuery("""
                INSERT INTO todo_audit_log
                    (event_id, aggregate_id, title, event_type, occurred_at, recorded_at)
                VALUES (:eventId, :aggregateId, :title, :eventType, :occurredAt, :recordedAt)
                ON CONFLICT (event_id) DO NOTHING
                """);
        query.setParameter("eventId", log.getEventId());
